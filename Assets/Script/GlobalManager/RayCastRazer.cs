...
public class RayCastRazer : MonoBehaviour {
	...
	void Update () {
				...
				if (!selectedGameObject && !viewingGameObject && !selectedPuzzle) {
						if (Physics.Raycast (hand.transform.position, hand.transform.TransformDirection (Vector3.forward), out rayHit, rayCastLenght)) {
								...
								else if (rayHit.collider.tag == "Puzzle") {
								// Si on n'est plus sur un objet on lui rend sa couleur normale, sinon on ne fait rien
									viewPuzzle = rayHit.collider.gameObject;
						}	
	...
	void ViewPuzzle(){
		if (SixenseInput.Controllers [0].GetButton (SixenseButtons.BUMPER)) {
			selectedPuzzle=viewPuzzle;
			if(selectedPuzzle.transform.parent) {
				if (selectedPuzzle.transform.parent.name != "HandR") {
					selectedGameObjectParent = selectedPuzzle.transform.parent;
				}
			}
			selectedPuzzle.transform.parent = hand.transform.GetChild(1).transform.parent;
			Vector3 pos = selectedPuzzle.transform.position;
			pos.z = 0;
			selectedPuzzle.transform.position=pos;
			Quaternion angle = selectedPuzzle.transform.rotation;
			Quaternion angle2 = selectedPuzzle.transform.localRotation;
			angle2.Set(0,0,0,1);
			angle.Set(0,0,0,1);
			selectedPuzzle.transform.rotation =angle;
